<div align="center">

<img width="120" height="120" alt="logo" src="https://github.com/user-attachments/assets/27ba2674-a9a5-4b97-9294-6ed12ceb1cd8" />


  # Pixel Music

  ### Material 3 Expressive Music Player

  **Ad free, beautifully designed music streaming.**

  A modern Android music player with YouTube Music integration, local file playback, synced lyrics, offline downloads, and a clean Material 3 Expressive interface.

  [![License: GPL v3](https://img.shields.io/badge/License-GPL_v3-blue.svg)](https://www.gnu.org/licenses/gpl-3.0)
  [![Platform](https://img.shields.io/badge/platform-Android-green.svg)](https://www.android.com)
  [![GitHub release](https://img.shields.io/github/v/release/zlgskt6/Pixel-Music?color=orange)](https://github.com/zlgskt6/Pixel-Music/releases)
  [![GitHub stars](https://img.shields.io/github/stars/zlgskt6/Pixel-Music?style=social)](https://github.com/zlgskt6/Pixel-Music/stargazers)

  [**📥 Download**](https://github.com/zlgskt6/Pixel-Music/releases/latest) • [**🐛 Report Bug**](https://github.com/zlgskt6/Pixel-Music/issues)

  </div>

  ---

  ## ✨ Features

  ### 🎧 Playback
  - Ad-free streaming with background listening
  - Multiple account support with quick switching
  - Local file & playlist support
  - Fast startup, lightweight performance
  - EBU R128 loudness normalization
  - Tempo, pitch, and playback speed controls
  - Crossfade between tracks
  - System equalizer
  - Optional option to sign in on Youtube & Spotify

  ### 🎤 Lyrics & Discovery
  - Live synced lyrics
  - Built in music recognition feature
  - Real-time listening statistics
  - Import playlists from Spotify
  - YouTube Music account sync

  ### 🎨 Design
  - Material 3 Expressive design language
  - Album-art powered dynamic colors
  - Smooth animations throughout the app.
  - Clean browsing, player, artist, album, and lyrics views
  - And of course, dark mode. (i personally prefer working with the app in light mode)
  - Super noob friendly!

  ---

  ## 📱 Screenshots

<img width="240" height="520" alt="Screenshot_20260906_234455" src="https://github.com/user-attachments/assets/1e353d9e-ae52-4341-a73b-55e4936470ee" />
<img width="240" height="520" alt="Screenshot_20260906_234644" src="https://github.com/user-attachments/assets/cc573590-ecc2-4131-af22-e1db54ad780f" />
<img width="240" height="520" alt="Screenshot_20260906_234618" src="https://github.com/user-attachments/assets/a353bee9-a53d-4bf1-bd9d-f5578fd01346" />
<img width="240" height="520" alt="Screenshot_20260906_234553" src="https://github.com/user-attachments/assets/e7044739-9e08-4572-bdca-5e25752079e9" />
<img width="240" height="520" alt="Screenshot_20260906_234534" src="https://github.com/user-attachments/assets/8d7027fb-441a-41c2-ba6c-cc13ad87a01e" />
<img width="240" height="520" alt="Screenshot_20260906_234508" src="https://github.com/user-attachments/assets/c6a1115b-3584-4f05-9d06-fa7ae63ec9db" />


  ---

### I'd appreciate some feedback
This app has only been tested and used on my Pixel 10 Pro XL, would love to hear how mid range devices run
Thank you

  ---

  ## 📥 Installation

  ### 🔽 Direct APK
  1. Go to [Releases](https://github.com/zlgskt6/Pixel-Music/releases/latest)
  2. Download the APK for your device (arm64 recommended for most phones)![Uploading Screenshot_20260906_234455.png…]()

  3. Install on your Android device (enable "Install from unknown sources")

  ---

  ## 🛠️ Building from Source

  ### Requirements
  - Android Studio Ladybug or newer
  - JDK 21
  - Android SDK 37

  ### Steps
  ```bash
  git clone https://github.com/zlgskt6/Pixel-Music.git
  cd Pixel-Music
  ./gradlew assembleGmsMobileUniversalRelease --no-configuration-cache
  ```

  ---

  ## 📄 License

  This project is licensed under the **GNU General Public License v3.0** — see [LICENSE](LICENSE) for details.

  

  ## ❤️ Credits & Inspiration

  This project acknowledges and credits @rukamori/ArchiveTune as a primary source of inspiration for design and feature ideas. They've done all the heavy work behind the scenes, such as the foundation of the UI and Youtube algorithm work and deserve much love.

  - Repository: https://github.com/rukamori/ArchiveTune
  - Author: @rukamori
  - Base framework: ArchiveTune (https://github.com/rukamori/ArchiveTune)

  Thank you to rukamori & co for the inspiration; parts of the UI/UX. Just let me know if there's something.

  

  © 2026 zlgskt6

  </div>
